...
${Object.entries(TABELA_COMPLETA.animais)
  .map(([id, nome]) => `${nome} (Grupo ${id}): dezenas ${TABELA_COMPLETA.grupos[nome].join(', ')}`)
  .join('\n')}
...
export function promptEmergencia(input, modalidade) {
  ...
  return {
    ...
    justificativa: `🔮 INTERPRETAÇÃO MÍSTICA: Sua mensagem começa com a palavra "${primeiraPalavra}". Analisando sua vibração energética através da fonética quântica, esta palavra ressoa com o ${animalFonetico} (Grupo ${grupoFonetico}).`,
    confianca: 'baixa',
    variacoes: {
    ...
    curiosidade:
      'A cultura popular brasileira acredita que palavras com sons guturais remetem a animais terrestres, enquanto sons agudos remetem a aves.',
  };
}
